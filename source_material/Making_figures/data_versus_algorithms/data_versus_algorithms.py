...
# ---------------------------------------------
# Visually extracted / approximated data points
# (read off the original JPG; values rounded)
# ---------------------------------------------
x = np.array([0.25, 0.5, 1, 5, 10, 50, 100, 250, 500, 1000])  # “Millions of words”

memory_based   = np.array([0.828, 0.842, 0.858, 0.88, 0.889, 0.902, 0.920, 0.933, 0.938, 0.945])
winnow         = np.array([0.75, 0.82, 0.832, 0.895, 0.91, 0.955, 0.956, 0.9568, 0.97, 0.975])
perceptron     = np.array([0.79, 0.82, 0.84, 0.885, 0.91, 0.948, 0.947, 0.96, 0.965, 0.97])


naive_bayes    = np.array([0.81, 0.82, 0.85, 0.91, 0.92, 0.94, 0.945, 0.955, 0.961, 0.965])

# ---------------------------------------------
# Re‑plot with matplotlib defaults
# ---------------------------------------------
plt.figure(figsize=(6.5, 3))

plt.plot(x, memory_based,  marker='o',linestyle='-',  label='memory‑based',   )
plt.plot(x, winnow,        marker='x',linestyle='--',  label='winnow',       )
plt.plot(x, perceptron,    marker='^',linestyle=':',  label='perceptron',   )
plt.plot(x, naive_bayes,   marker='s',linestyle='-.',  label='naïve Bayes',  )

plt.xscale('log')
plt.xlabel('millions of words')
plt.ylabel('test accuracy')
# plt.xtickslabel([0.1, 1, 10, 100, 1000])
plt.xticks([0.1, 1, 10, 100, 1000],   # positions  (must be numeric)
           ['0.1', '1', '10', '100', '1000'])  # labels     (strings)
plt.xlim(0.1, 1300)
plt.ylim(0.74, 1.0)
plt.legend(framealpha=1,loc=4)
plt.grid(True)
plt.tight_layout()
plt.savefig("data_versus_algorithms",dpi=300)
