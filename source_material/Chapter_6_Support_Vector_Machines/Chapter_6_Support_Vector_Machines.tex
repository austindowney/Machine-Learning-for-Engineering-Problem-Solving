...
\end{equation}
If the outcome is positive, the predicted class $\hat{y}$ is the positive class (1); otherwise, it is the negative class (0).
...
For data that follow an approximately linear trend, the \texttt{LinearSVR} class in \textsc{Scikit-Learn} solves the primal problem directly. It scales in $O(mn)$ time with $m$ samples and $n$ features, making it a practical choice for large data sets where a simple linear fit is adequate.
