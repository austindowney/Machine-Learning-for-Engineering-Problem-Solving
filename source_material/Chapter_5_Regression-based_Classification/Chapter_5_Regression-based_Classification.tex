...
\begin{equation}
    \hat{p} = h_\theta(X) = \sigma(\theta^{\top}{T} \cdot X).
\end{equation}
...
\end{equation}
Note that $\sigma(x) < 0.5$ when $x < 0$, and $\sigma(x) \ge 0.5$ when $x \ge 0$. Therefore, the Logistic Regression model predicts 0 if $\theta^{\top} \cdot X$ is negative, and 0 if it is positive.
...
\begin{equation}
\frac{\partial J}{\partial \theta_j} ;=; \frac{1}{m} \sum_{i=1}^{m} \Bigl[\sigma \bigl(\theta^{\top}X^{(i)}\bigr) - y^{(i)}\Bigr],x^{(i)}_{j}.
...
\begin{equation}
s_k(\textbf{x}) = \textbf{x}^{\top}\pmb{\theta}^{(k)}.
\end{equation}
...
\begin{equation}
\hat{y} = \mathop{\text{argmax}}_{k} \sigma \big(\textbf{s}(\textbf{x})\big)_k = \mathop{\text{argmax}}_{k} s_k(\textbf{x}) = \mathop{\text{argmax}}_{k} \big( (\theta^{(k)})^{\top} \cdot \textbf{x} \big).
\end{equation}
...
\pagebreak
\includepdf[pages=-, width= 0.95\textwidth, pagecommand = {\subsection{Examples} \subsubsection*{Example 5.1}\vspace{0.5em}}]{../code/example_5.1_Iris_data_set.pdf}

\includepdf[pages=1, width= 0.95\textwidth, pagecommand = {\subsubsection*{Example 5.2}\vspace{0.5em}}]{../code/example_5.2_Iris_1D_decision_boundary.pdf}
\includepdf[pages=2, width= 0.95\textwidth, pagecommand = {\vspace{0.5em}}]{../code/example_5.2_Iris_1D_decision_boundary.pdf}

\includepdf[pages=1, width= 0.95\textwidth, pagecommand = {\subsubsection*{Example 5.3}\vspace{0.5em}}]{../code/example_5.3_Iris_2D.pdf}
\includepdf[pages=2, width= 0.95\textwidth, pagecommand = {\vspace{0.5em}}]{../code/example_5.3_Iris_2D.pdf}

\includepdf[pages=1, width= 0.95\textwidth, pagecommand = {\subsubsection*{Example 5.4}\vspace{0.5em}}]{../code/example_5.4_softmax.pdf}
\includepdf[pages=2, width= 0.95\textwidth, pagecommand = {\vspace{0.5em}}]{../code/example_5.4_softmax.pdf}
