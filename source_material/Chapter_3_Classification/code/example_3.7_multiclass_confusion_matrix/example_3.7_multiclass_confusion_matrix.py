...
# next look at only the noise in the system, to do this, divide each value in 
# the confusion matrix by the number of images in the corresponding class, so 
# you can compare error rates instead of the absolute number of errors (which would
# make abundant classes look unfairly bad):
row_sums = conf_mx.sum(axis=1, keepdims=True)
norm_conf_mx = conf_mx / row_sums


# Next, we remove the high values along the diagonal. This is done by converting
# the confusion matrix to a float data type, and replacing # everything on the
# diagonal with NaNs.
