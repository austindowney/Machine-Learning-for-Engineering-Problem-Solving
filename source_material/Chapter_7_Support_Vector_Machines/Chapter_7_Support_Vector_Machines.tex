...
\begin{equation}
\textbf{w}^{\top} \textbf{x} + b = w_1 x_1 + \cdots + w_n x_n + b.
\end{equation}
...
\begin{equation}
  \hat{y} = 
  \begin{cases}
  0 & \text{if } \textbf{w}^{\top} \textbf{x} + b < 0, \\
  1 & \text{if } \textbf{w}^{\top} \textbf{x} + b \ge 0.
  \end{cases}
...
\begin{equation}
t^{(i)}(\textbf{w}^{\top} \textbf{x}^{(i)} + b) \geq 1
\end{equation}
for all training instances. This forms the basis of the hard margin linear SVM classifier optimization problem:

\begin{equation}
\begin{split}
    & \underset{\textbf{w}, b}{\text{minimize}} \;\;\;\; \frac{1}{2}\textbf{w}^{\top}\textbf{w} \\
    & \text{subject to} \;\;\;\; t^{(i)}(\textbf{w}^{\top}\textbf{x}^{(i)} + b) \ge 1 \; \text{for} \; i = 1, 2, \ldots, m
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
    & \underset{\textbf{p}}{\text{minimize}} \;\;\;\; \frac{1}{2}\textbf{\textbf{p}}^{\top}\textbf{H}\textbf{p} + \textbf{f}^{\top}\textbf{p}\\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=6.5in]{../figures/SVM_nonlinear_illustration}
	\caption{Nonlinear SVM example and illustration that shows: (a) 2D data that is not linearly separable , and (b) the same data plotted in a transformed feature space such that is is now linearly separable.}
...
%In the dual form of an SVM the inner product $\mathbf w^{\top}\mathbf x$ is replaced by a kernel
%\begin{equation}
%k(\mathbf x,\mathbf x),
%\end{equation}
...
\pagebreak
\includepdf[pages=1, width= 0.95\textwidth, pagecommand = {\subsection{Examples} \subsubsection*{Example 7.1}\vspace{0.5em}}]{../code/example_7.1_Linear_Classification.pdf}
\includepdf[pages=2, width= 0.95\textwidth, pagecommand = {\vspace{0.5em}}]{../code/example_7.1_Linear_Classification.pdf}

\includepdf[pages=1, width= 0.95\textwidth, pagecommand = {\subsubsection*{Example 7.2}\vspace{0.5em}}]{../code/example_7.2_polynomial_features.pdf}


\includepdf[pages=1, width= 0.95\textwidth, pagecommand = {\subsubsection*{Example 7.3}\vspace{0.5em}}]{../code/example_7.3_SVM_Polynomial_kernel.pdf}


\includepdf[pages=1, width= 0.95\textwidth, pagecommand = {\subsubsection*{Example 7.4}\vspace{0.5em}}]{../code/example_7.4_SVM_regression.pdf}
