...
\begin{itemize}
    \item $X$ represents the input to the system (i.e., the data).
    \item $y$ denotes the output from the system (i.e., classified data used for training).
    \item $\hat{p}$ signifies the estimated probability that an instance $x$ belongs to the positive class.
    \item $\hat{y}$ denotes the predicted output based on the classified data.
    \item $C(\theta)$ is a piecewise function for one data point.
    \item $J(\theta)$ represents the cost function for the entire dataset.
    \item $h_\theta$ signifies the hypothesis to be tested.
    \item $\sigma(\cdot)$ stands for the sigmoid function, also referred to as the logistic function or logit.
\end{itemize}
...
Here, $\hat{p}$ denotes the estimated probability, $h_\theta(X)$ represents the hypothesis that $X$ belongs to a class given the parameters $\theta$, and $X$ denotes the matrix of input features plus a bias term. Lastly, the logistic function also termed the logit, denoted by $\sigma(\cdot)$, is a sigmoid function (i.e., S-shaped) that yields a value between 0 and 1. It is defined as illustrated in Equation~\ref{eq:logistic_function} and depicted in Figure~\ref{fig:sigmoid_function}.
...
Once the Logistic Regression model has estimated the probability $\hat{p} = h_\theta(X)$ that an instance $X$ belongs to the positive class, it can easily make its prediction $\hat{y}$ (see Equation~\ref{eq:model_prediction}).
...
\begin{figure}[H]
	...
	\caption{classification threshold}
...
The overall cost function, denoted as $J(\theta)$ over the entire training set, represents the average cost across all training instances. It can be expressed in a single equation, known as the log loss, depicted in Equation~\ref{eq:log_loss}.
...
\begin{example}
Let's perform an example using the Iris dataset. Figure~\ref{fig:iris_species} displays flowers of three distinct iris plant species. First, let's explore the dataset.
\end{example}


\subsubsection{1-D Decision Boundaries}

Now, let's attempt to construct a classifier to identify the Iris-Virginica type solely based on the petal width feature. First, let's load the data:
...
\begin{example}
Do example on Iris data set 1D decision boundary example
\end{example}
...
\begin{example}
Do example on Iris data set 2D decision boundary example.

Let's attempt to construct a classifier to identify the Iris-Virginica species solely based on the petal width feature.
...
Once the scores for every class have been computed for the instance $\textbf{x}$, the model estimates the probability $\hat{p}_k$ that the instance belongs to class $k$ by passing the scores through the softmax function (refer to Equation~\ref{eq:softmax}): it exponentiates every score and then normalizes them (by dividing each by the sum of all exponentials).
...
Here:
\begin{itemize}
\item $K$ denotes the number of classes.
\item $s(\textbf{x})$ represents a vector containing the scores of each class for the instance $\textbf{x}$.
\item $\sigma \big(s(\textbf{x})\big)_k$ signifies the estimated probability that the instance $\textbf{x}$ belongs to class $k$, given the scores of each class for that instance.
\end{itemize}

Similar to the Logistic Regression classifier, the Softmax Regression classifier predicts the class with the highest estimated probability (which corresponds to the class with the highest score).

\begin{equation}
\hat{y} = \mathop{\text{argmax}}_{k} \sigma \big(s(\textbf{x})\big)_k = \mathop{\text{argmax}}_{k} s_k(\textbf{x}) = \mathop{\text{argmax}}_{k} \big( (\theta^{(k)})^\text{T} \cdot \textbf{x} \big)
\end{equation}
...
\begin{figure}[H]
	...
	\caption{Softmax classification for the three three iris plant species.}
...
\begin{example}
	Do example on softmax classification.
\end{example}
