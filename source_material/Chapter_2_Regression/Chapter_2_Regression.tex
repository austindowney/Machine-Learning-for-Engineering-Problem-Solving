...
\begin{equation}
J(\textbf{X}, h_\theta) = J = \frac{1}{m} \sum_{i=1}^{m} (\pmb{\theta}^{\top}\textbf{x}^{(i)} - y^{(i)})^2
\end{equation}
...
\begin{equation}
\frac{\partial}{\partial \theta_j} J(\theta) = \frac{2}{m} \sum_{i=1}^{m}(\pmb{\theta}^{\top}\textbf{x}^{(i)} - y^{(i)}) x_j^{(i)}
...
\begin{equation}
	...
	\end{pmatrix}
	= \frac{2}{m} \textbf{X}^{\top} \cdot (\textbf{X} \pmb{\theta} - \textbf{y})
\end{equation}
