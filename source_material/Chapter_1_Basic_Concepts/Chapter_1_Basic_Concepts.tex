...
\begin{figure}[t]
    \centering
    \includegraphics[width=2in]{../figures/Symbolics3640_Modified.jpg}
...
\footnotetext[1]{Michael L. Umbricht and Carl R. Friend (Retro-Computing Society of RI), CC BY-SA 3.0 $<$https://creativecommons.org/licenses/by-sa/3.0$>$, via Wikimedia Commons}
	...
	\begin{data}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=5.5in]{../figures/imageNet_classificatoin_paper}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=\linewidth]{../figures/ImageNet-competition-results}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=\linewidth]{../figures/Nobel}
	...
	\subsection{Definition of Machine Learning}
		...
		\begin{itemize}
			\item	Machine Learning is the science (and art) of programming computers so they can learn from data. Here is a slightly more general and more engineering-oriented definitions:
			\begin{itemize}
			...
			\begin{itemize}
				\item From Hands on ML with sk learn: ``x(i) is a vector of all the feature values (excluding the label) of the $i$\textsuperscript{th} instance in the dataset, and y(i) is its label (the desired output value for that instance).''
		...
		\footnotetext[1]{Langs, G., Röhrich, S., Hofmanninger, J. et al., CC BY 4.0 $<$https://creativecommons.org/licenses/by/4.0$>$, via Wikimedia Commons}

	\subsection{Supervision in Machine Learning}


		The general domains of ML can be classified into supervised and unsupervised learning.

		
		\subsubsection{Supervised Learning}
			
			\textbf{Supervised}, the data is pre-categorized with labels
			\begin{itemize}
			...
			In supervised learning, the training data provided to the algorithm includes the desired solutions, known as labels. A common task within this type of learning is classification. An example of classification is a spam filter, which is trained using numerous emails that are each labeled as either spam or ham. The objective for the spam filter is to learn how to accurately classify new emails based on this training.

			
			\begin{figure}[t]
				\centering
				\includegraphics[width=\linewidth]{../figures/Supervised_machine_learning_in_a_nutshell.png}
				\caption{Supervised machine learning in a nutshell\protect\footnotemark[1].}
				\label{fig:training_set}
			\end{figure}

		\footnotetext[1]{EpochFail, CC BY-SA 4.0 $<$https://creativecommons.org/licenses/by-sa/4.0$>$, via Wikimedia Commons}
			...
			\begin{itemize}
				\item	Regression is another typical task is to predict a target numeric value, such as the price of a house, given a set of features (size, proximity to railroad or highways, age of house, style, etc.) called predictors.
				\begin{itemize}
					\item	To train a system you need to build a model and train in on many house sale examples, including both predictors (house features) and their labels (i.e., prices).
				\end{itemize}
			\end{itemize}
			...
			Here are some of the most important supervised learning algorithms (covered in this class):
			\begin{itemize}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=4in]{../figures/A_semantic_word_cloud_of_Barack_Obama's_First_Inaugural_Address}
				\caption{A semantic word cloud of Barack Obama's First Inaugural Address \protect\footnotemark[1].}
				\label{fig:data_visualization}
			\end{figure}

		\footnotetext[1]{GuoYongzhi, CC BY-SA 4.0 $<$https://creativecommons.org/licenses/by-sa/4.0$>$, via Wikimedia Commons}

			
		\subsubsection{Unsupervised Learning }

		
\textbf{Unsupervised}, the data is not labeled in any way
			\begin{itemize}
...
Visualization algorithms serve as an instance of unsupervised learning algorithms: they take in extensive, complex, and unlabeled data and produce a 2D or 3D representation that can be conveniently visualized. These algorithms strive to maintain as much of the original data structure as possible (for example, ensuring that distinct clusters in the input space do not merge in the visual output). This helps in comprehending the organizational structure of the data and potentially uncovering hidden patterns.
			...
			\begin{figure}[t]
				\centering
				\includegraphics[width=2in]{../figures/iris_no_label}
				\caption{iris flower that will be used throughout this class  \protect\footnotemark[1]}
				\label{fig:iris}
			\end{figure}			
		\footnotetext[1]{No machine-readable author provided. Dlanglois assumed (based on copyright claims)., CC BY-SA 3.0 $<$http://creativecommons.org/licenses/by-sa/3.0/$>$, via Wikimedia Commons }	
		...
		\footnotetext[1]{Agor153, CC BY-SA 3.0 $<$https://creativecommons.org/licenses/by-sa/3.0$>$, via Wikimedia Commons}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=3.5in]{../figures/iris_k-means.png}
				\caption{Principal tree for Iris data set \protect\footnotemark[1]}
				\label{fig:iris_k-means}
			\end{figure}	
...
\begin{itemize}
\item	Some important visulation items we will look at in class incude. 
				\begin{itemize}
...
\end{itemize}		
			...
			\begin{itemize}
				\item	Another crucial unsupervised learning task is anomaly detection. This includes identifying irregular credit card transactions to combat fraud, detecting defects in manufacturing, or removing outliers from a dataset prior to processing it with another learning algorithm. The system is trained on what normal instances look like. When it encounters a new instance, it assesses whether the instance resembles the norm or appears to be an anomaly.
			\end{itemize}
		
		\subsubsection{Semisupervised Learning}
			
			Certain algorithms are designed to manage partially labeled training data, typically characterized by a substantial amount of unlabeled data with a minimal amount of labeled data.
		...
		\subsubsection{Reinforcement Learning}
			...
			Reinforcement Learning is a very different concept. The system learns using an agent where the can:
				\begin{itemize}
					\item	observe the environment
					\item 	select and perform actions
					\item 	get rewards in return (or penalties)
					\item 	learns by itself what is the best strategy, called a policy. 
					\item 	tries to get the most reward over time.
				\end{itemize}
			
			
			A policy specifies the action that an agent should take when presented with a particular situation.
...
Online learning enables the incremental training of the system by sequentially feeding it data instances, either individually or in small groups called mini-batches. Each learning step is rapid and economical, allowing the system to update its knowledge continuously as new data arrives. Online learning is particularly suitable for systems that:
\begin{itemize}
...
A critical parameter in online learning systems is the learning rate, which dictates how rapidly the system adapts to changing data:
\begin{itemize}
    \item A high learning rate allows for rapid adaptation but may also lead to quick forgetting of old data and training on noise,
    \item A low learning rate results in slower learning and reduced sensitivity to variations in new data.
\end{itemize}
		...
		A significant challenge in online learning is the system's susceptibility to performance degradation when exposed to poor quality data. This is particularly problematic in live systems where clients might quickly notice issues. For instance, bad data could originate from a malfunctioning robot sensor or an attempt to manipulate search engine rankings through spamming. To mitigate these risks, it is crucial to vigilantly monitor the system and quickly disable learning or revert to a previously effective state if a decline in performance is observed. Additionally, monitoring the input data for anomalies and employing anomaly detection algorithms can help identify and respond to aberrant data.
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=\linewidth]{../figures/cows}
				...
				\label{fig:cows}
			\end{figure}
			\footnotetext[1]{Screen shoot of google image search for cows taken by the Austin Downey, all images stated to be under a creative commons license per Google search tools; also assumed to be fair use under given the educational purpose of this text, via \url{https://www.google.com/}}
...
This method exemplifies instance-based learning, where the system memorizes specific examples and uses a similarity metric to generalize to new instances.
...
Another approach to generalization involves constructing a model based on a set of examples and then using this model to make predictions. This method is known as model-based learning.
			
			
			\begin{figure}[H]
			\centering
				\includegraphics[width=\linewidth]{../figures/model_based_learning}
...
One of the initial challenges faced by newcomers to machine learning is selecting the appropriate algorithms (or estimators) for specific tasks. This is due to the fact that various algorithms excel with different kinds of data and problems. The flowchart depicted in Fig.~\ref{fig:ml_map}, which originates from the scikit-learn library documentation, is intended to guide users in choosing the most suitable algorithms for their particular datasets.
			...
			\begin{figure}[H]
			\centering
				\includegraphics[width=\linewidth]{../figures/ml_map.png}
				\caption{Flowchart of estimators used in the scikit learn library that intends guide users for what algorihtms to use for a given case \protect\footnotemark[1].}
				\label{fig:ml_map}
			\end{figure}
...
In a landmark study released in 2001, Microsoft researchers Michele Banko and Eric Brill demonstrated that a variety of Machine Learning algorithms, even relatively straightforward ones, achieved similar levels of performance on a complex task of natural language disambiguation when provided with sufficient data. This finding is illustrated in figure~\ref{fig:data_versus_algorithms} and further discussed in their paper \protect\footnotemark[2].

\footnotetext[2]{Michele Banko and Eric Brill. ``Scaling to very very large corpora for natural language disambiguation.'' Proceedings of the 39th annual meeting of the Association for Computational Linguistics. 2001.}

			\begin{figure}[H]
			\centering
				\includegraphics[width=4in]{../figures/data_versus_algorithms.png}
				\caption{Learning Curves for four algorithms showing the importance of data versus algorithms.  \protect\footnotemark[1]}
				\label{fig:data_versus_algorithms}
			\end{figure}


			As the authors put it: ``these results suggest that we may want to reconsider the tradeoff between spending time and money on algorithm development versus spending it on corpus development.''  ``The Unreasonable Effectiveness of Data'' \protect\footnotemark[2]. It's important to recognize, however, that small and medium-sized datasets remain prevalent, and acquiring additional training data is not always straightforward or economical. Therefore, the significance of algorithm selection should not be overlooked.

\footnotetext[2]{Image used under US Fair use polices, originally published by Michele Banko and Eric Brill in ``Scaling to very very large corpora for natural language disambiguation.'' Proceedings of the 39th annual meeting of the Association for Computational Linguistics. 2001.}
